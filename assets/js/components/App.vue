            ...
            <div id="header">
                <div class="headerbox">
                    ...
                    <div class="search-area">
                        ...
                        <div data-tilt class="search-button" v-on:click="searchActive = true">Zoeken</div>
                    </div>
                </div>
            </div>
            <div class="row" v-if="searchActive">
                ...
                <ais-no-results>
                    <template slot-scope="{ query }">
                        No products found for <i>{{ query }}</i>.
                    </template>
                </ais-no-results>
                    <ais-results>
                        <template slot-scope="{ result }">
                            <div class="search_results">
                                ...
                                <div class="search_result" v-if="index == 'Training'">
                                    <a :href="'/trainers/' + result.user.slug">
                                        <div class="avatar">
                                            <img class="avatar" src="">
                                            <div class="trainer">
                                                <span>{{ result.user.firstName }} {{ result.user.lastName }}</span>
                                                <span class="functie">{{ result.user.typeTrainer }} {{ result.user.lastName }}</span>
                                            </div>
                                        </div>
                                    </a>
                                    <a :href="'/trainers/' + result.slug">
                                        <div class="content">
...
<script>
    ...
    export default {
        name: 'app',
        props: ['appId', 'apiKey', 'indexSuffix'],
        data() {
            return {
                index: 'Training',
        ...
        computed: {
            ...
            searchActive: {
                get: function() { return this.searchStore._stoppedCounter === 0 },
